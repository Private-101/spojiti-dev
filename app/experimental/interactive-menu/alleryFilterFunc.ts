...
export const allergyFilter = (
    ...
    menu.forEach((eachDish) => {
      ...
      if (!containsAllergen) {
        allergyFreeMenu.push(eachDish);

      }
  
      if (containsRemovableAllergen && !containsAllergen) {
    ...
    removables.forEach((removableItem) => {
      if (allergyFreeMenu.includes(removableItem)) {
        ...
        allergyFreeMenu.push({
          dishName: `**Contains ${removableItem.removable}** ${removableItem.dishName} (can be modified to fit your preferences 👩🏽‍🍳)`,
          course: removableItem.course,
          integral: removableItem.integral,
          removable: removableItem.removable.map((item) => `**Remove** ${item}`),
